...
### Added
- **全新动态路由中枢** — Dashboard 控制面板全面进化：移除预设推荐限制，新增「路由规划设定」分区，赋予用户将特定任务流完全指派至指定专家模型的能力。
- **创意写作协作链** — 开放核心生产管线的自定义配置，支持在「大纲生成」、「初稿撰写」、「智能打磨」、「逻辑复检」等核心阶段自由挂载所需的外部大模型集群。

### Changed
- **系统核心预设库升级** — 模型注册表全线换代至最新生产力标准：全面采用 GPT-5.4、Claude 4.6 (Opus/Sonnet)、Gemini 3.1 家族、DeepSeek V3、Qwen 3.5 与 GLM-5 为底层标尺。
- **文档规约重构** — 核心说明规范化，彻底确立 L-Hub 为「外部大模型扩展中枢」定位，移除可能产生歧义的产品关联字眼，专精 Antigravity IDE 深层融合体验。
